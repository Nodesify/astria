...
// / relationships[] — the same shape upstream graphify accepts, not the full
...
pub fn parse_scip(scip_path: &Path, text: &str) -> Result<Extraction> {
    let doc: serde_json::Value = serde_json::from_str(text).map_err(|e| GraphifyError::Parse {
    ...
    if nodes.is_empty() {
        return Err(GraphifyError::Parse {
            file: scip_path.display().to_string(),
...
pub fn parse_scip_file(scip_path: &Path) -> Result<Extraction> {
    let text = std::fs::read_to_string(scip_path).map_err(|e| GraphifyError::Parse {
        file: scip_path.display().to_string(),
    ...
    fn scip_round_trips_symbols_and_relationships() {
        let json = r#"{
            "documents": [{
                ...
                "language": "rust",
                "symbols": [
                    {"symbol": "graphify::Graph", "display_name": "Graph", "kind": "Class",
                     "relationships": [
                        {"symbol": "graphify::GraphLike", "is_implementation": true},
                        {"symbol": "graphify::Node", "is_type_definition": true}
                     ]},
                    {"symbol": "graphify::GraphLike", "display_name": "GraphLike"}
                ]
            }]
        }"#;
        let ext = parse_scip(Path::new("index.scip.json"), json).unwrap();
        // Two symbol entries; graphify::Node only appears as a relationship
        ...
        // scip_typed points at graphify::Node — a different symbol, so a
