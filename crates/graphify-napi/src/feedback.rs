...
pub fn save_result(
    ...
    let memory_dir = graphify_dir.join("memory");
    ...
    db.execute(
        ...
        rusqlite::params![
            ...
            graphify_paths::normalize(&memory_path),
...
/// Aggregate memory docs into `graphify-out/reflections/LESSONS.md`.
pub fn reflect(graphify_dir: &Path) -> Result<String> {
    let memory_dir = graphify_dir.join("memory");
    ...
    let lessons_dir = graphify_dir.join("reflections");
...
mod tests {
    ...
    fn save_result_persists_doc_node_and_edges() {
        ...
        let gdir = dir.path().join(".graphify");
